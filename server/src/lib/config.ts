...
export const config = {
  ...
  /**
   * Open Food Facts — открытая база упакованных продуктов. Региональный поддомен отдаёт то,
   * что продаётся в России. Подменяется в smoke на локальную заглушку; пусто — поиск выключен.
   */
  offApiBase: (process.env.OFF_API_BASE ?? 'https://ru.openfoodfacts.org').replace(/\/+$/, ''),
};
