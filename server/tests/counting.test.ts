...
test('вступивший позже: полная норма, и тренировка до вступления засчитана', () => {
  // Правило из computeWeek: и норма, и зачёт — по всей неделе челленджа. Позднее вступление
  // не значит, что человек не тренировался: данные с часов приходят за прошедшие дни,
  // а в приложение он заходит позже.
  const week = { start: '2026-10-05', end: '2026-10-11', days: 7 };
  const joined = clampWindow(week, '2026-10-08'); // вступил в четверг, до конца недели 4 дня
  assert.ok(joined);

  const workouts = [
    w('2026-10-05T06:00:00Z', 65), // до вступления
    w('2026-10-08T06:00:00Z', 65),
  ];

  assert.equal(joined.days, 4);
  // по окну участия вышло бы 2 — так и было, пока норму не сделали одной для всех
  assert.equal(requiredFor(3, joined.days), 2);
  assert.equal(requiredFor(3, week.days), 3);
