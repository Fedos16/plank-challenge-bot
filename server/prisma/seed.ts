...
const WEIGHT_RULES_TEXT = `Взвешивание

1. Штрафов, дедлайнов и кружков здесь нет — это просто общая группа для тех, кто следит за весом
2. Вес и состав тела приходят с телефона вместе с тренировками: умные весы пишут их в
   Apple Health или Health Connect, а приложение-хаб пересылает сюда
3. Весы без такого приложения не беда — взвешивание можно внести вручную
4. История веса личная: другие участники видят только имя, но не ваши цифры`;
...
async function main() {
  ...
  // Челлендж-группа для взвешиваний: ни кружков, ни дедлайнов, ни штрафов — только состав
  // участников (кому можно отдать профиль весов) и личная история веса у каждого.
  const weight = await prisma.challenge.upsert({
    where: { key: 'weight' },
    update: {},
    create: {
      key: 'weight',
      kind: 'weight',
      title: 'Взвешивание',
      description: 'Вес с умных весов: своя история и динамика. Без штрафов и кружков.',
      rulesText: WEIGHT_RULES_TEXT,
      isActive: true,
      timezone: process.env.TZ ?? 'Europe/Moscow',
      startDate,
    },
